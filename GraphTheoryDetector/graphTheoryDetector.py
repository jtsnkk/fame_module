...
class GraphTheoryDetector(PreprocessingModule):
    name = "GraphTheoryDetector"
    ...
    def each_with_type(self, target, target_type):
        ...
        #output=subprocess.run(["python3", path+"/GraphTheoryDetector/MD_Model/detector.py", "--model", target], capture_output=True, text=True)
        cmd = ("python3", path+"/GraphTheoryDetector/MD_Model/detector.py", "--model", target)
