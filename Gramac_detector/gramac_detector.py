...
class Gramac_detector(PreprocessingModule):
    ...
    def each_with_type(self, target, target_type):
        ...
        #output=subprocess.run(["python3", path+"/Gramac_detector/model/detector.py", "--model", target], capture_output=True, text=True)
        cmd = ("python3", path+"/Gramac_detector/model/detector.py", "--model", target)
        ...
        if "BenignWare" in output:
            output = "BenignWare"
        elif "Malware" in output:
            output = "Malware"
        else:
            output = "error"
